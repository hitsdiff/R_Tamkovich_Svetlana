shinyServer(function(input,output){
  #vars <- reactiveValues(transaction_data=transaction_data)
  
  
  #for (i in length(transaction_data_less$TRANS_TIME)) {
  #n<- 4-str_length(transaction_data_less$TRANS_TIME[i])
  
  #transaction_data_less$zeros[i]<- paste(replicate(n, "0"),transaction_data_less$TRANS_TIME[i], collapse = "")
  ...
  #n<- 4-str_length(transaction_data_less$TRANS_TIME[45])
  #n
  #paste(replicate(n, "0"),transaction_data_less$TRANS_TIME[45], collapse = "")
  ...
  #output$time_table <- renderRHandsontable({
   # transaction_data_less <- select(transaction_data,household_key,QUANTITY,STORE_ID,PRODUCT_ID,SALES_VALUE,TRANS_TIME)  #,SALES_VALUE,QUANTITY)
    ...
    #rhandsontable(head(transaction_data_less,40))
  #})
